...
const Page=(props)=>{
    ...
    const test_excel=async()=>{
        const req=await axios.get("/tor-rab.xlsx", {responseType:"arraybuffer"})
        ...
        const row=sheet.getRow(55).values.map(v=>{
            if(!_.isNull(v)){
                if(_.isObject(v)){
        ...
        const row_uniq=[...new Set(row)]
        console.log(row_uniq.join(" "))
    }
    ...
    return (
        <Modal show={props.data.is_open} onHide={props.toggleTambah} backdrop="static" size="md" scrollable>
            <Formik
                ...
                validationSchema={
                    yup.object().shape({
                        file_tor:yup.string().required(),
                        file_rab:yup.string().required(),
                    ...
                    <form onSubmit={formik.handleSubmit}>
                        ...
                        <Modal.Body>
                            <div className="row">
                                <div className="col-12">
                                    <div className="mb-2">
                                        <label className="my-1 me-2" for="country">File TOR <span className="text-danger">*</span></label>
                                        <div>
                                            <label>
                                                ...
                                                <input
                                                    ...
                                                    onChange={e=>{
                                                        ...
                                                        .then(data=>{
                                                            formik.setFieldValue("file_tor", data.data.file)
                                                        })
                                                        .catch(err=>{
                                            ...
                                            <button
                                                ...
                                                disabled={formik.values.file_tor=="" || formik.values.is_generating}
                                                onClick={async e=>{
                                                    const url=`/storage/${formik.values.file_tor}`
                                                    const exts=url.split(/[#?]/)[0].split('.').pop().trim()

                                                    let data
                                                    formik.setFieldValue("is_generating", true)
                                                    if(exts=="pdf"){
                                                        data=await parse_pdf(url)
                                                    }
                                                    else{
                                                    ...
                                                    if(!data.error){
                                                        for(var i=0; i<form.length; i++){
                                                            if(data.searcher[i].index!=-1){
                                                                formik.setFieldValue(form[i], data.data[i].right_block)
                                                            }
                                                        }
                                                    }
                                                }}
                                            >
                                                {formik.values.is_generating?<>Generating...</>:<>Generate OCR</>}
                                            </button>
                                        </div>
                                        <div className="mt-1">
                                            {formik.values.file_tor!=""?
                                                <a href={`/storage/${formik.values.file_tor}`} target="_blank" className="text-dark text-truncate">{formik.values.file_tor}</a>
                                            :
                                        ...
                                        <div className="form-text text-muted mt-2">
                                            <ol>
                                                <li>Pilih file JPG, PNG, atau PDF. jika file PDF generate OCR akan diambil di halaman 1!</li>
                                                <li>Format/Template <a href="/tor.pdf" target="_blank">JPG/PDF</a></li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-12">
                                    <div className="mb-2">
                                        <label className="my-1 me-2" for="country">File RAB <span className="text-danger">*</span></label>
                                        <div>
                                            <label>
                                                <div className="btn btn-secondary btn-sm cursor-pointer">
                                                    <FiUpload className="icon"/> Upload
                                                ...
                                                <input
                                                    ...
                                                    onChange={e=>{
                                                        file_request.uploadDokumen(e.target.files[0])
                                                        .then(data=>{
                                                            formik.setFieldValue("file_rab", data.data.file)
                                                        })
                                                        .catch(err=>{
                                                            if(err.response.status===401){
                                                                router.visit("/")
                                        ...
                                        <div className="mt-1">
                                            {formik.values.file_rab!=""?
                                                <a href={`/storage/${formik.values.file_rab}`} target="_blank" className="text-dark text-truncate">{formik.values.file_rab}</a>
                                            :
                                                <span className="text-dark">Tidak ada berkas dipilih!</span>
                                            }
                                        </div>
